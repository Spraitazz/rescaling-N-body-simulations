...
double z_to_t(double z) {
	...
	F.params = 0;  	
	gsl_integration_qagiu(&F, z, 0, 1e-7, 1000, z_to_t_workspace, &result, &error);	
	return result;	
}
...
int prep_redshift_spline(double zmin, double zmax, int bins) {
	redshift_spline_reversed = prep_spline_generic(zmin, zmax, bins, false, true, &z_to_t);
	return 0;
}
...
SplineInfo Dplus_spline(int gravity, double k) {
	...
	//dont want output to file here, so 3rd parameter is ""
