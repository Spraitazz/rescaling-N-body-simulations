...
int toRedshift() {		
	for (int i = 0; i < particle_no; i++) {		
		particles[i][2] += particles[i][5] / (a*H);	//z-axis line of sight		
...
int overdensity_pk(bool gaussian) {
    ...
    kmax = nyquist[0] * sqrt(3.0) * Jenkins_foldfactor; 
    	...
    	for (int bb = 0; bb < cells[1]; bb++) {
    		for (int cc = 0; cc < cells[2]; cc++) {
				...
				if (flag) {
					if (gaussian) {
						//NOT YET ADDED 
						//pk_logbin_Gaussian(index, k, logkmin, logkmax, logdk, mu);
						//pk_logbin(index, k, logkmin, logkmax, logdk, mu);
					} else {
						if (k >= kmin && k < kmax) {
							pk_logbin(index, k, logkmin, logkmax, logdk, mu);
						}
...
/*
//get rid of unused bins
int remove_bins() {	
	int real_ind;
	real_ind = 0;
	for (int i = 0; i < spectrum_size; i++) {
		if (toRemove_bins[i] == false) {
			power_spectrum_monopole[real_ind] = power_spectrum_monopole[i];
			power_spectrum_quadrupole[real_ind] = power_spectrum_quadrupole[i];
			bin_k_average[real_ind] = bin_k_average[i];
			k_number[real_ind] = k_number[i];
			real_ind += 1;			
		}
	}		
	//pad the rest with 0s
	for (int i = 0; i < spectrum_size - real_ind; i++) {
		power_spectrum_monopole[spectrum_size - i - 1] = 0.0;
		power_spectrum_quadrupole[spectrum_size - i - 1] = 0.0;
		bin_k_average[spectrum_size - i - 1] = 0.0;
		k_number[spectrum_size - i - 1] = 0;
	}
	spectrum_size_final = real_ind + 1;
	return 0;
}
*/
...
/*
int pk_logbin_Gaussian(int index, double k, double logkmin, double logkmax, double logdk, double mu) {
 	
 	double expectation, magnitude, phase;                   
    double logk, fk_squared, coef_real, coef_im, L2, inv_no_density;
	int index_log_pk;
	
    expectation = (1. + mu*mu/2.) * k;

    // expectation                    *= spherical_tophat(kmodulus, 8.)*spherical_tophat(kmodulus, 8.)*pow(app_sigma8, -2.)*pow(   bsigma8,  2.);


    //double Power                              = -log(gsl_rng_uniform(gsl_ran_r))*expectation;
    
    // Note AMPLITUDE IS NOT the expectation. 
    // amplitude                          = sqrt(Power);
    //amplitude                          = sqrt(expectation);
    logk = log10(k);
    if (logk >= logkmin && logk < logkmax) {					

		
		magnitude = gsl_ran_exponential(gsl_ran_r, expectation);
		// phase = gsl_ran_flat(gsl_ran_r, 0., 2.0*M_PI);
		
		fk_squared = magnitude;
		
		//plot is VP(k) versus k
		fk_squared *= volume;
		//shot noise
		fk_squared -= inv_no_density;			
	
		L2 = 0.5 * (3. * mu * mu - 1.);	
						
		index_log_pk = (int) floor((logk - logkmin)/logdk);	
