...
#ks = np.delete(ks, index)
ks1 = ks[:18]
ks2 = ks[22:]
mean_mono_1 = mean_mono[:18]
mean_mono_2 = mean_mono[22:]
err_on_mean_mono_1 = err_on_mean_mono[:18]
err_on_mean_mono_2 = err_on_mean_mono[22:]
#mean_mono = np.delete(mean_mono, index)
#err_on_mean_mono = np.delete(err_on_mean_mono, index)

#pl.errorbar(ks, mean_mono, yerr=err_on_mean_mono, label=r"$P_{0}(k)$")
pl.errorbar(ks1, mean_mono_1, yerr=err_on_mean_mono_1, color="b", label=r"$P_{0}(k)$")
pl.errorbar(ks2, mean_mono_2, yerr=err_on_mean_mono_2, color="b", label=r"$P_{0}(k)$")
pl.plot(data_model[:,0], data_model[:,1], color="g", label=r"model $P_{0}(k)$")
#pl.errorbar(ks, mean_noise, yerr=err_on_mean_noise, label="centrals monopole")

pl.legend(loc="best")

pl.ylim(4e2,5.7e2)
pl.xlim(0.95*kmin, 0.7*kmax)

pl.ylabel(r"$P(k), \ [(h^{-1} \rm{Mpc})^3]$")
pl.xlabel(r"$k, \ [h \rm{Mpc} ^{-1}]$")


pl.savefig("./onehalo_meh.pdf", bbox_inches='tight')
