...
rows = file_len("./onehalo/randoms_onehalo_0.dat")
#print rows
data = []
monos = np.zeros(rows)
...
for i in xrange(0, simno):
	this_data = np.loadtxt("./onehalo/randoms_onehalo_" + str(i) + ".dat")
	#print data[:,1]
	monos += this_data[:,1]
	quadros += this_data[:,2]
...
ax.set_xscale("log")
ax.set_yscale("log")
ax.set_title("One-halo term for randoms")
#pl.
pl.errorbar(ks, monos, yerr=err_monos, label="monopole")
...
pl.savefig("./onehalo/onehalo_average.pdf")
pl.clf()
