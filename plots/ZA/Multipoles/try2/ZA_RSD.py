...
ks = data[:,0]
data_mono = data[0:34,1]
data_quadro = abs(data[0:34,2])
data_hex = abs(data[0:34,3])

for i in xrange(1, simno):
	print i
	this_data = np.loadtxt("./ZA_RSD_pk_" + str(i + startind) + ".dat")
	data_mono = np.vstack((this_data[0:34,1], data_mono))
	data_quadro = np.vstack((abs(this_data[0:34,2]), data_quadro))
	data_hex = np.vstack((abs(this_data[0:34,3]), data_hex))	
