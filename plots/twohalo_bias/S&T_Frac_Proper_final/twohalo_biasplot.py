...
data_mono_debiased = data_debiased[0:34,1]
data_mono_debiased_mmin = data_debiased_mmin[0:34,1]
data_mono_debiased_mmax = data_debiased_mmax[0:34,1]
...
for i in xrange(1, simno):
	...
	data_mono_debiased = np.vstack((this_data_debiased[0:34,1], data_mono_debiased))
	data_mono_debiased_mmin = np.vstack((this_data_debiased_mmin[0:34,1], data_mono_debiased_mmin))
	data_mono_debiased_mmax = np.vstack((this_data_debiased_mmax[0:34,1], data_mono_debiased_mmax))
...
pl.plot(data_model_bveff[:,0], data_model_bveff[:,1], color="Aqua", label=r"model $P_{0}(k)$ (all haloes)")
pl.errorbar(ks, mean_mono_debiased, yerr=err_on_mean_mono_debiased, linestyle="None", color="Blue", label=r"measured $P_{0}(k)$ (all haloes)")

pl.plot(data_model_bvmin[:,0], data_model_bvmin[:,1], color="DeepPink", label=r"model $P_{0}(k)$ ($10^{12} M_{\odot}$ haloes)")
pl.errorbar(ks, mean_mono_debiased_mmin, yerr=err_on_mean_mono_debiased_mmin, linestyle="None", color="Red", label=r"measured $P_{0}(k)$ ($10^{12} M_{\odot}$ haloes)")

pl.plot(data_model_bvmax[:,0], data_model_bvmax[:,1], color="GreenYellow", label=r"model $P_{0}(k)$ ($10^{13} M_{\odot}$ haloes)")
pl.errorbar(ks, mean_mono_debiased_mmax, yerr=err_on_mean_mono_debiased_mmax, linestyle="None", color="Green", label=r"measured $P_{0}(k)$ ($10^{13} M_{\odot}$ haloes)")
...
#pl.plot(data_model_debiased[:,0], data_model_debiased[:,1], label="model debiased (S&T HMF)")
